...
public class Rune {
  ...
  public Collection<Coordinates> getParts() {
    int startZ = coords.z - Blink.halfRuneSize;
    int startX = coords.x - Blink.halfRuneSize;
  ...
  public boolean isPart(World otherWorld, Coordinates otherLoc) {
    return world.equals(otherWorld)
        && otherLoc.y == coords.y && otherLoc.z >= coords.z - Blink.halfRuneSize && otherLoc.z <= coords.z + Blink.halfRuneSize
        && otherLoc.x >= coords.x - Blink.halfRuneSize && otherLoc.x <= coords.x + Blink.halfRuneSize;
  }
